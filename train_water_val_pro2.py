...
class MetricsCallback(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        # 获取训练指标
        train_loss = get_metric(['train/loss_epoch', 'train/loss'])
        train_precision = get_metric(['train/precision'])
        train_recall = get_metric(['train/recall'])
        train_f1 = get_metric(['train/f1'])
        train_miou = get_metric(['train/miou', 'train/iou/water_epoch', 'train/iou/water'])
        
        # 获取验证指标
        val_loss = get_metric(['val/loss'])
        val_precision = get_metric(['val/precision'])
        val_recall = get_metric(['val/recall'])
        val_f1 = get_metric(['val/f1'])
        val_miou = get_metric(['val/miou', 'val/iou/water'])
...
def train_water(args):
    ...
    try:
        ...
        metrics_callback = MetricsCallback(log_save_path, model_info, args)
